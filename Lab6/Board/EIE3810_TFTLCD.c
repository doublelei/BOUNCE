...
void EIE3810_TFTLCD_WhiteBackground(void){
	EIE3810_TFTLCD_WrCmd(0X2c00);
	for (int pos=0; pos<480*800; pos++){
		EIE3810_TFTLCD_WrData(WHITE);
	}
}
void EIE3810_TFTLCD_Clear(u16 color){
	if (color == WHITE){
		EIE3810_TFTLCD_WhiteBackground();
	}
}
...
{
	EIE3810_TFTLCD_SegmentSelect(EIE3810_SegmentBitEncoder(digit),start_x,start_y,color);
}

void EIE3810_TFTLCD_SegmentSelect(u16 binNum, u16 start_x, u16 start_y, u16 color)
{
	start_y -= 140;
	if ((binNum & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x,10,start_y+10,55,color);//Bit 0: segment 0
	if (((binNum>>1) & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x,10,start_y+75,55,color);//Bit 1: segment 1
	if (((binNum>>2) & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x+10,55,start_y+130,10,color);//Bit 2: segment 2
	if (((binNum>>3) & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x+65,10,start_y+75,55,color);//Bit 3: segment 3
	if (((binNum>>4) & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x+65,10,start_y+10,55,color);//Bit 4: segment 4
	if (((binNum>>5) & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x+10,55,start_y,10,color);//Bit 5: segment 5
	if (((binNum>>6) & 0x1) == 1) EIE3810_TFTLCD_FillRectangle(start_x+10,55,start_y+65,10,color);//Bit 6: segment 6
}
u16 EIE3810_SegmentBitEncoder(u16 numDisplay)
{
	if (numDisplay == 0) return 0x3F;
	if (numDisplay == 1) return 0x18;
	if (numDisplay == 2) return 0x76;
	if (numDisplay == 3) return 0x7C;
	if (numDisplay == 4) return 0x59;
	if (numDisplay == 5) return 0x6D;
	if (numDisplay == 6) return 0x6F;
	if (numDisplay == 7) return 0x38;
	if (numDisplay == 8) return 0x7F;
	if (numDisplay == 9) return 0x7D;
}

u16 EIE3810_TFTLCD_ColorSelect(u8 b, u16 color, u16 bgcolor)
{
	if (b) return color;
	else return bgcolor;
}
...
void EIE3810_TFTLCD_ShowInt(u16 x, u16 y, int Num, u16 color, u16 bgcolor)
{
	...
	if (Num<10000)
	{
		EIE3810_TFTLCD_FillRectangle(x,40,y,16,WHITE);
		int digit;
		u16 lastX = x + (4-1)*8;
		for (int i=0;i<4;i++)
		{
			digit = Num % 10;
			Num /= 10;
			EIE3810_TFTLCD_ShowChar(lastX-8*i,y,digit+48,color,bgcolor);
		}
	}else{
		EIE3810_TFTLCD_ShowStr(x+40,y,"+",color,bgcolor);
	}
}
...
void EIE3810_TFTLCD_ShowChar(u16 x, u16 y, u8 ascii, u16 color, u16 bgcolor)
{
	...
	u16 represent[16];
	for (int i=0;i<16;i++){
		represent[i] = asc2_1608[ascii-32][i];
	}
	int col;
	for (int i=0; i<16; i++){
		u8 byt = represent[i];
		for (int j=0; j<8; j++){
			...
			if ((i%2)==0){
				col = i/2;
				EIE3810_TFTLCD_DrawDot(x+col,y+(7-j),EIE3810_TFTLCD_ColorSelect((byt>>j)&0x1,color, bgcolor));
			}

			//Lower byte:
			if ((i%2)==1){
				col = (i-1)/2;
				EIE3810_TFTLCD_DrawDot(x+col,y+(7-j)+8,EIE3810_TFTLCD_ColorSelect((byt>>j)&0x1,color, bgcolor));
...
/**********  16*8  fonts   ***********/
void EIE3810_TFTLCD_ShowStr(u16 x, u16 y, char* str, u16 color, u16 bgcolor)
{
	int i = 0;
	while(str[i]!= 0)
	{
		EIE3810_TFTLCD_ShowChar(x+8*i, y, str[i], color, bgcolor);
		i += 1;
	}
}

/**********  24*12  fonts   ***********/
void EIE3810_TFTLCD_ShowStr2412(u16 x, u16 y, char* str, u16 color, u16 bgcolor)
{
	int i = 0;
	while(str[i]!= 0)
	{
		EIE3810_TFTLCD_ShowChar2412(x+12*i, y, str[i], color, bgcolor);
		i += 1;
	}
}
